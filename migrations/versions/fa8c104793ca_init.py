"""init

Revision ID: fa8c104793ca
Revises: 
Create Date: 2023-08-23 21:51:37.898767

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = 'fa8c104793ca'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table_schema("STG_TEST")
    op.create_table_schema("STG_UNION_MEMBER")
    op.create_table(
        'union_member',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('type_of_learning', sa.String(), nullable=False),
        sa.Column('rzd_status', sa.String(), nullable=False),
        sa.Column('academic_level', sa.String(), nullable=False),
        sa.Column('status', sa.String(), nullable=False),
        sa.Column('faculty', sa.String(), nullable=False),
        sa.Column('first_name', sa.String(), nullable=False),
        sa.Column('last_name', sa.String(), nullable=False),
        sa.Column('email', sa.String(), nullable=False),
        sa.Column('date_of_birth', sa.String(), nullable=False),
        sa.Column('phone_number', sa.String(), nullable=False),
        sa.Column('image', sa.String(), nullable=False),
        sa.Column('rzd_datetime', sa.String(), nullable=False),
        sa.Column('rzd_number', sa.String(), nullable=False),
        sa.Column('grade_level', sa.Integer(), nullable=False),
        sa.Column('has_student_id', sa.Boolean(), nullable=False),
        sa.Column('entry_date', sa.String(), nullable=False),
        sa.Column('status_gain_date', sa.String(), nullable=False),
        sa.Column('card_id', sa.Integer(), nullable=False),
        sa.Column('card_status', sa.String(), nullable=False),
        sa.Column('card_date', sa.String(), nullable=False),
        sa.Column('card_number', sa.String(), nullable=False),
        sa.Column('card_user', sa.String(), nullable=False),
        sa.Column('card', sa.String(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        schema='STG_UNION_MEMBER',
    )
    op.create_table(
        'test', sa.Column('id', sa.Integer(), nullable=False), sa.PrimaryKeyConstraint('id'), schema='STG_TEST'
    )
    op.create_table(
        'new_test',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        schema='STG_UNION_MEMBER',
    )
    op.create_group("dwh_STG_TEST_read")
    op.grant_rights("dwh_STG_TEST_read", "STG_TEST.test")
    op.create_group("dwh_STG_TEST_write")
    op.grant_rights("dwh_STG_TEST_write", "STG_TEST.test")
    op.create_group("dwh_STG_TEST_all")
    op.grant_rights("dwh_STG_TEST_all", "STG_TEST.test")
    op.create_group("dwh_STG_UNION_MEMBER_read")
    op.grant_rights("dwh_STG_UNION_MEMBER_read", "STG_UNION_MEMBER.union_member")
    op.grant_rights("dwh_STG_UNION_MEMBER_read", "STG_UNION_MEMBER.new_test")
    op.create_group("dwh_STG_UNION_MEMBER_write")
    op.grant_rights("dwh_STG_UNION_MEMBER_write", "STG_UNION_MEMBER.union_member")
    op.grant_rights("dwh_STG_UNION_MEMBER_write", "STG_UNION_MEMBER.new_test")
    op.create_group("dwh_STG_UNION_MEMBER_all")
    op.grant_rights("dwh_STG_UNION_MEMBER_all", "STG_UNION_MEMBER.union_member")
    op.grant_rights("dwh_STG_UNION_MEMBER_all", "STG_UNION_MEMBER.new_test")
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.revoke_rights("dwh_STG_UNION_MEMBER_all", "STG_UNION_MEMBER.new_test")
    op.revoke_rights("dwh_STG_UNION_MEMBER_all", "STG_UNION_MEMBER.union_member")
    op.delete_group("dwh_STG_UNION_MEMBER_all")
    op.revoke_rights("dwh_STG_UNION_MEMBER_write", "STG_UNION_MEMBER.new_test")
    op.revoke_rights("dwh_STG_UNION_MEMBER_write", "STG_UNION_MEMBER.union_member")
    op.delete_group("dwh_STG_UNION_MEMBER_write")
    op.revoke_rights("dwh_STG_UNION_MEMBER_read", "STG_UNION_MEMBER.new_test")
    op.revoke_rights("dwh_STG_UNION_MEMBER_read", "STG_UNION_MEMBER.union_member")
    op.delete_group("dwh_STG_UNION_MEMBER_read")
    op.revoke_rights("dwh_STG_TEST_all", "STG_TEST.test")
    op.delete_group("dwh_STG_TEST_all")
    op.revoke_rights("dwh_STG_TEST_write", "STG_TEST.test")
    op.delete_group("dwh_STG_TEST_write")
    op.revoke_rights("dwh_STG_TEST_read", "STG_TEST.test")
    op.delete_group("dwh_STG_TEST_read")
    op.drop_table('test', schema='STG_TEST')
    op.drop_table('new_test', schema='STG_UNION_MEMBER')
    op.drop_table('union_member', schema='STG_UNION_MEMBER')
    op.drop_table_schema("STG_TEST")
    op.drop_table_schema("STG_UNION_MEMBER")
    # ### end Alembic commands ###
