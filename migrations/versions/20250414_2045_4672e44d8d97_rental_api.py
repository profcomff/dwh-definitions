...
Revision ID: 4672e44d8d97
Revises: f8908a4837a5
Create Date: 2025-04-14 20:45:14.252311
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('strike_date', sa.DateTime(), nullable=False, comment='Timestamp начисления страйка, мск'),
        sa.Column('overdue_flag', sa.Boolean(), nullable=False, comment='Флаг просрочки'),
        sa.Column('strike_count', sa.Integer(), nullable=False, comment='Количество страйков у пользователя'),
        sa.PrimaryKeyConstraint('api_id'),
        schema='DM_RENTAL',
...
def downgrade():
    ...
    op.drop_table('dm_rentals', schema='DM_RENTAL')
    op.drop_table('dm_availability', schema='DM_RENTAL')
    op.delete_group(
        "test_dwh_stg_rental_all" if os.getenv("ENVIRONMENT") != "production" else "prod_dwh_stg_rental_all"
    )
    op.delete_group(
