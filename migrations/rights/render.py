...
def render_schema_grant(autogen_context, op):
    return f'op.grant_on_schema("test_{op.group_name}" if os.getenv("ENVIRONMENT") != "production" else "prod_{op.group_name}", ' \
           f'"{op.schema}")'


@renderers.dispatch_for(RevokeOnSchemaOp)
def render_schema_revoke(autogen_context, op):
    return f'op.revoke_on_schema("test_{op.group_name}" if os.getenv("ENVIRONMENT") != "production" else "prod_{op.group_name}", ' \
           f'"{op.schema}")'
...
def render_table_grant(autogen_context, op):
    ...
    return f'op.grant_on_table("test_{op.group_name}" if os.getenv("ENVIRONMENT") != "production" else "prod_{op.group_name}", ' \
           f'{op.scopes}, {table})'
...
def render_table_revoke(autogen_context, op):
    table = f"""'"{op.table_name.split(".")[0]}".{op.table_name.split(".")[1]}'"""
    return f'op.revoke_on_table("test_{op.group_name}" if os.getenv("ENVIRONMENT") != "production" else "prod_{op.group_name}", ' \
           f'{op.scopes}, {table})'
