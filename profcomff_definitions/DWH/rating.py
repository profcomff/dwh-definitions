...
class LecturerRank(Base):
    ...
    mark_weighted: Mapped[int] = mapped_column(nullable=False, comment="Взвешенная оценка преподавателя")
    mark_kindness_weighted: Mapped[int] = mapped_column(nullable=False, comment="Взвешенная доброта преподавателя")
    mark_clarity_weighted: Mapped[int] = mapped_column(nullable=False, comment="Взверешенная понятность преподавателя")
    mark_freebie_weighted: Mapped[int] = mapped_column(nullable=False, comment="Взвешенная халявность преподавателя")
    valid_from_dt: Mapped[date | None] = mapped_column(comment="Дата начала действия записи")
    valid_to_dt: Mapped[date | None] = mapped_column(comment="Дата конца действия записи")
